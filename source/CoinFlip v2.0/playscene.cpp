...
PlayScene::PlayScene(int levelNum, int mode){
    ...
    QString str1 = "";
    if(this->modIndex != 2){
        str1 = QString("Level: %1").arg(this->levelIndex);
    }else{
        str1 = QString("随机模式");
    }
    ...
    }else if(this->modIndex == 1){
        for(int i = 0; i < 6; i++){
            for(int j = 0; j < 6; j++){
                this->gameArray[i][j] = config.mData[this->levelIndex+20][i][j];

            }
        }
    }else if(this->modIndex == 2){
        for(int i = 0; i < 6; i++){
            for(int j = 0; j < 6; j++){
                this->gameArray[i][j] = config.mData[41][i][j];
            }
        }
    }
    ...
    if(this->modIndex == 0){
        for(int i = 0; i < 4; i++){
            for(int j = 0; j < 4; j++){
                ...
                coin->setParent(this);
                coin->setFixedSize(pix.width()-5, pix.height()-5);
                coin->move(230 + 50*i, 204 + 50*j);
                ...
                connect(coin, &MyCoin::clicked,[=](){
                    ...
                    QTimer::singleShot(200, this, [=](){
                        //翻转右侧硬币
                        if(coin->posX+1 <= 3 && coinBtn[coin->posX+1][coin->posY]->flag != 2){
                            coinBtn[coin->posX+1][coin->posY]->changFlag();
                            this->gameArray[coin->posX+1][coin->posY] = this->gameArray[coin->posX+1][coin->posY] == 0 ? 1 : 0;
                        }
                        //翻转左侧硬币
                        if(coin->posX-1 >= 0 && coinBtn[coin->posX-1][coin->posY]->flag != 2){
                            coinBtn[coin->posX-1][coin->posY]->changFlag();
                            this->gameArray[coin->posX-1][coin->posY] = this->gameArray[coin->posX-1][coin->posY] == 0 ? 1 : 0;
                        }
                        //翻转上侧硬币
                        if(coin->posY-1 >= 0 && coinBtn[coin->posX][coin->posY-1]->flag != 2){
                            coinBtn[coin->posX][coin->posY-1]->changFlag();
                            this->gameArray[coin->posX][coin->posY-1] = this->gameArray[coin->posX][coin->posY-1] == 0 ? 1 : 0;
                        }
                        //翻转下侧硬币
                        if(coin->posY+1 <= 3 && coinBtn[coin->posX][coin->posY+1]->flag != 2){
                            coinBtn[coin->posX][coin->posY+1]->changFlag();
                            this->gameArray[coin->posX][coin->posY+1] = this->gameArray[coin->posX][coin->posY+1] == 0 ? 1 : 0;
                        }
            ...
            for(int j = 0; j < 6; j++){
                ...
                if(coin->flag != 2){
                    connect(coin, &MyCoin::clicked,[=](){
                        ...
                        QTimer::singleShot(200, this, [=](){
                            //翻转右侧硬币
                            if(coin->posX+1 <= 5 && coinBtn[coin->posX+1][coin->posY]->flag != 2){
                                coinBtn[coin->posX+1][coin->posY]->changFlag();
                                this->gameArray[coin->posX+1][coin->posY] = this->gameArray[coin->posX+1][coin->posY] == 0 ? 1 : 0;
                            }
                            //翻转左侧硬币
                            ...
                            //翻转下侧硬币
                            if(coin->posY+1 <= 5 && coinBtn[coin->posX][coin->posY+1]->flag != 2){
                                coinBtn[coin->posX][coin->posY+1]->changFlag();
                                this->gameArray[coin->posX][coin->posY+1] = this->gameArray[coin->posX][coin->posY+1] == 0 ? 1 : 0;
                            }
